...
function extractBounds(boxModel: BoxModel): ElementBounds {
...
function checkOffscreen(bounds: ElementBounds, viewport: LayoutViewport): DiagnosticIssue[] {
...
function checkVisibility(styleMap: Map<string, string>): DiagnosticIssue[] {
  const issues: DiagnosticIssue[] = [];
...
function checkModalIssues(styleMap: Map<string, string>, bounds: ElementBounds, viewport: LayoutViewport): DiagnosticIssue[] {
  const issues: DiagnosticIssue[] = [];
...
function checkOverflow(styleMap: Map<string, string>): DiagnosticIssue[] {
  const issues: DiagnosticIssue[] = [];
  ...
  try {
    ...
    if (name === "diagnose_layout") {
      ...
      try {
        ...
        styleMap = new Map(
          computedStyle.map((prop: any) => [prop.name, prop.value])
        );
      } catch (error: any) {
    ...
    if (name === "check_responsive") {
      const selector = args?.selector as string;
      const breakpoints = args?.breakpoints as number[] || [320, 768, 1024, 1920];
