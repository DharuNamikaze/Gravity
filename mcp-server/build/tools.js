...
async function getParentChain(nodeId, maxDepth) {
    const chain = [];
    ...
    for (let i = 0; i < maxDepth; i++) {
        try {
            ...
            chain.push({
                ...
                className: node.node.attributes?.find((a, i) => a === "class")
...
async function getChildren(nodeId, maxDepth) {
    try {
        ...
        return (node.node.children || []).map((child) => {
            const classIndex = child.attributes?.findIndex((a) => a === "class");
            return {
                nodeId: child.nodeId,
...
export async function checkAccessibility(selector) {
    ...
    const issues = [];
    const styleMap = new Map(styles.computedStyle.map((s) => [s.name, s.value]));
    // Check ARIA attributes
    const ariaLabelIndex = node.node.attributes?.findIndex((a) => a === "aria-label");
    const ariaLabel = ariaLabelIndex !== undefined && ariaLabelIndex >= 0 ? node.node.attributes[ariaLabelIndex + 1] : null;
    const ariaRoleIndex = node.node.attributes?.findIndex((a) => a === "role");
    ...
    try {
        ...
        return {
            ...
            summary: {
                ...
                byType: groupBy(listeners.listeners || [], (l) => l.type)
            }
        };
    }
    catch (e) {
...
function hasIssue(bounds, styleMap, viewport, issueType) {
    const THRESHOLD = 2;
    switch (issueType) {
...
async function getZIndex(nodeId) {
    try {
        const styles = await sendCDPCommand("CSS.getComputedStyleForNode", { nodeId });
        const styleMap = new Map(styles.computedStyle.map((s) => [s.name, s.value]));
        const zIndexStr = styleMap.get("z-index") || "auto";
...
async function getAllElements(nodeId, elements = []) {
    try {
        const node = await sendCDPCommand("DOM.describeNode", { nodeId });
        const classIndex = node.node.attributes?.findIndex((a) => a === "class");
